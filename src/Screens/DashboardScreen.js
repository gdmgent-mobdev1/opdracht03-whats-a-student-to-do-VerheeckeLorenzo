...
export default class DashboardScreen extends Screen {
    ...
    async render() {
        ...
        this.invites = await Invite.getInvites(this.userId);
        // if there are any notifications, create a notification with the notification API
        if (this.invites.length > 0) {
            this.invites?.forEach(async (invite) => {
                const notification = new Notification("Student's To Do", {
                    body: `You have been invited to a project by ${invite.from}`,
                    icon: "📋",
                });
            });
        }
        ...
        const header = ElementFactory.createHeader({ headerText: "Student's To Do", user: this.user, invites: allInvites });
        // Create a calendar
        ...
        if (this.projects.length === 0) {
            ...
            this.projects.forEach((project) => {
                ...
                const projectTitle = ElementFactory.createTitle({ classNames: ["projectTitle"], text: project.title, size: 2 });
        ...
        const createProjectTitle = ElementFactory.createTitle({ classNames: ["createProjectTitle"], text: "Create a new project", size: 2 });
        ...
        const timerButton = ElementFactory.createButton({ className: "timerButton", text: "Time a new task" });
